...
def update_charts(filter_dre):
    ...
    df_walk_company = etl_instance.etl_walk_company(filter_dre)
    ...
    fig_barchart.update_layout(
        ...
        yaxis=dict(tickformat=",.0f"),  # Format y-axis tick labels with commas
    ...
    # Fixar a primeira linha como um valor absoluto
    df_walk_company.loc[0, 'Delta'] = abs(df_walk_company.loc[0, 'Delta'])

    # Set of legend positive or negative values
    results = ['(+) Gross Revenue', '(=) Net Revenue','(=) Gross Profit',
    '(=) EBITDA', '(+) Financial Charges','(=) Net Result']
    if filter_dre in results:
    ...
    # Criar gráfico de waterfall
    fig_walk_company = go.Figure(go.Waterfall(
        x=df_walk_company['Company'],
        measure=['absolute'] + ['relative'] * (len(df_walk_company) - 2) + ['total'],
        y=df_walk_company['Delta'],
        base=0,
        decreasing=d_results,
        increasing=i_results,
        totals={"marker": {"color": "#100a61", "line": {"color": "#100a61", "width": 0.3}}}
    ))
    fig_walk_company.update_layout(
        title=f"Waterfall Company By {filter_dre}",
        waterfallgap=0.3,
        xaxis_title="Company",
        yaxis_title="Delta",
        xaxis_showgrid=False,  # Set showgrid to False for x-axis
    ...
    return fig_barchart, fig_barchart_upper, fig_walk_company
